...
export const ResultsScreen: React.FC<ResultsScreenProps> = ({
  ...
  const clean = (text: string) =>
    text?.toString().trim().toLowerCase().replace(/[^\w\s]/g, "");
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="space-y-4">
            {questions.map((question, index) => {
              const userAnswer = answers[question.id];
              const correct = isCorrectAnswer(question.correctAnswer, userAnswer);
                ...
                <Card key={question.id} className="shadow-soft border-0">
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <div className="text-sm text-muted-foreground mb-1">Your Answer:</div>
                        <div className={`font-medium ${correct ? 'text-success' : 'text-destructive'}`}>
                          {userAnswer || 'No answer provided'}
                        </div>
                      </div>
        ...
        {/* Restart */}
        <div className="text-center">
          <Button onClick={onRestart} variant="hero" size="lg" className="min-w-48">
            <RefreshCw className="h-5 w-5 mr-2" />
            Take Another Quiz
          </Button>
        </div>
